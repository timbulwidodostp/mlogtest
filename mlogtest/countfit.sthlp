...
{p_end}
{p2colset 5 18 19 0}
{synopt:{opt forcevuong}} Vuong test is not appropriate for testing zero-inflation. This
option forces the computation of the test. Greene (1994) proposed using the Vuong test for nonnested models (Vuong 1989) to test for zero
    inflation.  Despite many earlier citations, recent work by Wilson (2015) has shown that the Vuong
    test is inappropriate for testing zero inflation.  Nesting occurs when the probability of zero
    inflation is 0, which is on the boundary, and this violates the regularity conditions of the Vuong
    test for nonnested models.  So the distribution of the test statistic is not standard normal.  The
    actual distribution is unknown and thus cannot be used for inference.
{p_end}
{p2colset 5 18 19 0}
