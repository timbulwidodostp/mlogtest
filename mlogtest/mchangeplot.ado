*! version 2.0.0 2017-08-02 | long freese | stata 15 parameter naming fix

//  plot marginal effects computed by mchange

program define mchangeplot

    version 11.2
    capture drop _orme
    qui gen byte _orme = .
    label var _orme "Temporarily hold orplot and meplot information"
    char _orme[Cplottype] "meplot"

    * 2.0.0 2017-08-02 stata 15 parameter naming fix
    if (_caller()>=15) version 15: _orme_syntax `0'
    else _orme_syntax `0'

end
exit
* version 1.0.0 2014-02-20 | long | spost13 release
