...
*! version 4.1.0 2018-03-16 | long freese | coefplot code
*! TODO trap when to run mchangetocoefplot
...
program mchangetocoefplot, sclass

* TODO trap only do this for binary?
*    version 11.2

    capture matrix drop _mctocp
    capture matrix drop _mctocpvec
  
    tempname inmat
    tempname tempmat
    tempname vec
    *matrix `inmat' = r(changes)  
    matrix `inmat' = _mchange
    capture local tablencols = colsof(`inmat')
    if "`tablencols'"=="" {
    ...
    //  loop through matrix
    local innrows = rowsof(`inmat')
    local invars = `innrows'/8
    forvalues i = 1/`invars' {
        local st = ((`i'-1)*8) + 1
        local ime = `st' + 0
        local ilb = `st' + 4
        local iub = `st' + 5
        scalar me = `inmat'[`ime',2]
        scalar lb = `inmat'[`ilb',2]
        scalar ub = `inmat'[`iub',2]
        
        mat `vec' = (me, lb, ub)
        mat colnames `vec' = me lb ub

        * name of effect
        mat `tempmat' = `inmat'[`st',1...]
        local rownm  : rownames `tempmat'
        local roweq  : roweq    `tempmat'
        foreach amt in one_ sd_ rng_ marg_ {
            local ipos = strpos("`rownm'","`amt'")
            if `ipos'==1 {
                local amtis "`amt'"
                local amtis : subinstr local amtis "_" "", all
                local effectnm "`roweq'_`amtis'"    
            }
        }
        local ipos = strpos("`rownm'","bin_")
        if `ipos'==1 { // is bin
            local effectnm "`roweq'"    
            local effectnm : subinstr local effectnm "!" "_", all
        }
        
        mat rownames `vec' = `effectnm'
        mat roweq    `vec' = ""
        matrix       _mctocp = nullmat(_mctocp) \ `vec'
    }
...
* version 3.0.4 2017-08-01 | long freese | stata 15 _cons fix
* version 3.0.3 2015-04-18 | long freese | stat(ci p) allowed
