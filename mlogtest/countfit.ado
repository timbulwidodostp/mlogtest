*! version 3.2.2 2017-11-07 | long freese | forcevuong
...
program define countfit, rclass
    ...
    , [ Generate(string) stub(string) replace ///
      INFlate(string) /// inflation variables
      MAXcount(integer 9) ///
      NOGraph /// suppress lambert graph
      NODifferences /// suppress table of differences in probabilities
      NOEstimates /// suppress table of estimated parameters
      NOFit /// suppress table of fit statistics
      Prm Nbreg ZIP ZINb ///
      nodash /// no dash in output table
      NOConstant /// no constant
      NOPRTable ///
      forcevuong /// force the vuong test 2017-11-07
      note(string) /// note for graph
      GRAPHexport(string) /// options passed to graph export
      NOIsily NOSAVE ]

    if "`e(wtype)'"!="" {
        di in r "countfit does not work with weights."
        exit
    }

    if ("`stub'"!="") local generate "`stub'"

    if ("`nosave'"=="nosave") local generate "_no_save_"

    gettoken lhs rhs : varlist
    fvexpand `rhs'
    local rhs "`r(varlist)'"
    
    if ("`inflate'"== "") local inflate `rhs'
    else {
        fvexpand `inflate'
...
//  construct information on models being assessed
    ...
    if "`generate'"=="" local set "CF" // default name
...
//  table of estimates
    ...
    if "`noestimates'"=="" {
        estimates table `mdlnmlist', eform ///
            b(%9.3f) t(%7.2f) label varwidth(30) ///
            stats(alpha N ll bic aic)
    }

//  table of mean observed - predicted probabilities
    ...
    * loop through models and compute differences
    foreach m in `modellist' {
        ...
        * stats on mean absolute difference
        ...
        if ("`replace'"=="replace") capture drop `modelnm'absdif
...
// TABLES OF OBSERVED AND PREDICTED COUNTS
    ...
    if "`noprtable'" == "" {
        ...
        foreach t in `mdlnmlist' {
            ...
            local c4 = 40
            local c5 = 50
            di in y _new "`t'" in g ": Predicted and actual probabilities"
            di in g _new "Count" _col(`c1') "  Actual" ///
                _col(`c2') "Predicted" _col(`c3') "  |Diff|" ///
                _col(`c4') " Pearson"
            ...
            foreach c of numlist 0(1)`max' {
                ...
                di in y "`c'" _col(`c1') `f83' `ob' _col(`c2') `f83' `pr' ///
                              _col(`c3') `f83' `de' _col(`c4') `f83' `pe'

            } // loop through counts

            di in g "`dash'"
            di in y "Sum" _col(`c1') `f83' `sumob' _col(`c2') `f83' `sumpr' ///
                          _col(`c3') `f83' `sumde' _col(`c4') `f83' `sumpe'
...
// COMPARE FIT STATISTICS
    ...
    "-------------------------------------------------------------------------"
        ...
        di _new in g "Tests and Fit Statistics"
        di
        tempname aicd
...
di // 2017-11-07
di "Vuong test is not appropriate for testing zero-inflation. To force the"
di "the computation of the test, use option -forcevuong-."
    ...
    if "`nosave'"=="nosave" {
        capture drop _no_save_*
    }
    capture drop _est_`generate'*
    capture drop _est_PRM
    capture drop _est_NBRM
    capture drop _est_ZIP
    capture drop _est_ZINP
end
...
program define _count_estimate, rclass
    ...
    version 11.2
    syntax varlist(fv) [if] [in] [,     ///
      inflate(varlist fv) MAXcount(integer 9) Generate(string) replace ///
      Prm Nbreg ZIP ZINb NOConstant NOIsily ///
      forcevuong /// 2017-11-07
      ] 

    local vuong "" // 2017-11-07
    if "`forcevuong'"!="" local vuong "forcevuong"
...
// estimate models & create globals with stats
    ...
    tempname n
    local noi ""
    if ("`noisily'"=="noisily") local noi "noisily"
...
// prm
    ...
    if "`prm'"!="" {
        qui {
            ...
            `vv' ///
        `noi' poisson `varlist' `if' `in', `noconstant'
            ...
            qui mgen, stub(`fullnm') meanpred pr(0/`maxcount') `replace'
...
// nbreg
    ...
    if "`nbreg'"!="" {
        qui {
            ...
            local fullnm "`set'`modelnm'"
            `vv' ///
        `noi' nbreg `varlist' `if' `in', `noconstant'
            estimates store `fullnm'
            scalar ll`fullnm' = e(ll) // log lik
            ...
            mgen, stub(`fullnm') meanpred pr(0/`maxcount') `replace'
...
exit
* version 3.2.0 2014-02-14 | long freese | spost13 release
* version 3.2.1 2014-08-24 | long freese | inflate error
